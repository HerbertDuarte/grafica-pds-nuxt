...
const { data: tarefas, error, status, refresh } = useLazyFetch<Tarefa[]>("/api/tarefa/get-all");
...
const {
  itemsPerPage, currentPage, totalItems, totalPages, paginatedItems, changePage,
...
<template>
  <div class="space-y-3">
    ...
    <div class="flex items-center gap-2">
      <SearchInput v-model="busca" placeholder="Pesquisar tarefa" @search="realizarBusca" />
    </div>
    <Table>
      <TableHeader>
        ...
        <template v-else-if="paginatedItems?.length > 0">
          <TableRow v-for="tarefa in paginatedItems" :key="tarefa.id">
            ...
            <TableCell class="flex items-end justify-end w-[200px]">
              <div class="flex items-center gap-2">
                <TarefasUpdate :tarefa="tarefa" @tarefaAtualizada="refreshTarefas" />
                <TarefasDelete :id="tarefa.id" @tarefaDeletada="refreshTarefas" />
              </div>
            </TableCell>
          </TableRow>
        </template>
